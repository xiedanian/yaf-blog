...
--------------------------------------------------
 */
class Ald_Vender_EaseMob{
    private $client_id;
    private $client_secret;
    private $org_name;
    private $app_name;
    private $url;
    private $redis_key = 'easemob_token';
    private $cluster_name = 'common';
//------------------------------------------------------用户体系	
    /**
     * 初始化参数
     *
    ...
    function getToken()
    {
        $redis = Ald_Redis::getInstance($this -> cluster_name);
        $token = $redis -> get($this -> redis_key);
        if($token != false){
            return $token;
        }
        $options=array(
        ...
        $content = "Authorization:Bearer ".$tokenResult['access_token'];
        $expire = $tokenResult['expires_in'] - 3600; //提前一小时
        $redis -> setex($this -> redis_key, $expire, $content);
        return $content;
    }
    /**
    授权注册
